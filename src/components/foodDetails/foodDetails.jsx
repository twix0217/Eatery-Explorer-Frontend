...
const foodDetails = (props) => {
  ...
  useEffect(() => {
    async function getFood() {
      const foodData = await hootService.show(foodId);
      console.log(foodData);
      setRestaurant(foodData);
    }
    getFood();
  }, [restaurantsId]);

  const handleAddComment = async (formData) => {
    const newComment = await commentService.createFC(props.restaurantsId,foodId, formData);

    const copyFood = { ...food };
    copyFood.comments.push(newComment);

    setRestaurant(copyFood);
  };
